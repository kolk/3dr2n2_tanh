    ...
    def set_output(self):
        ...
        conv_out = conv.conv2d(
            ...
            image_shape=np.asarray(
                padded_input_shape, dtype=np.int64),
            border_mode='valid')
...
class ShapeConv3DLayer(Layer):
    """3D Convolution layer with 3D input and hidden unit"""
    ...
    def __init__(self, prev_layer, fc_layer, filter_shape, padding=None, params=None):
        ...
        self._x_filter_shape = [filter_shape[0],  # out channel
                              filter_shape[2],  # time
                              prev_layer._output_shape[2],  # in channel
                              filter_shape[3],  # height
                              filter_shape[4]]  # width
        self._padding = padding
        if padding is None:
...
class FCConv3DLayer(Layer):
    ...
    def __init__(self, prev_layer, fc_layer, filter_shape, padding=None, params=None, isTrainable = True):
        """Prev layer is the 3D hidden layer"""
        super().__init__(prev_layer)
        self._fc_layer = fc_layer
        self._filter_shape = [filter_shape[0],  # out channel
        ...
        if params is None:
            self.Wh = Weight(self._filter_shape, is_bias=False, train=isTrainable)
            ...
            self.b = Weight((filter_shape[0],), is_bias=True, mean=0.1, filler='constant', train=isTrainable)
            params = [self.Wh, self.Wx, self.b]
        else:
            self.Wh = params[0]
            self.Wx = params[1]
...
class FCConv1DLayer(Layer):
    ...
    def __init__(self, prev_layer, fc_layer, filter_shape, padding=None, params=None, isTrainable = True):
        ...
        super().__init__(prev_layer)
        self._fc_layer = fc_layer
        self._filter_shape = filter_shape  # out channel
        self._output_shape = [self._input_shape[0], self._filter_shape]

        if params is None:
            self._Wh_shape = [self._input_shape[1], self._filter_shape]

            self.Wh = Weight(
                self._Wh_shape,
                is_bias=False,
                fan_in=self._input_shape[1],
                fan_out=self._output_shape[1],
                train=isTrainable)

            self._Wx_shape = [self._fc_layer._output_shape[1], self._filter_shape]
            ...
            self.Wx = Weight(
                self._Wx_shape,
                is_bias=False,
                fan_in=self._fc_layer._output_shape[1],
                fan_out=self._output_shape[1],
                train=isTrainable)

            self.b = Weight((filter_shape,), is_bias=True, mean=0.1, filler='constant', train=isTrainable)
            params = [self.Wh, self.Wx, self.b]
        else:
            self.Wh = params[0]
            self.Wx = params[1]
    ...
    def set_output(self):
        fc_output = tensor.dot(self._fc_layer.output, self.Wx.val)
        prev_lay_out = tensor.dot(self._prev_layer.output, self.Wh.val)
        self._output = prev_lay_out + fc_output + self.b.val.dimshuffle( 'x', 0)
...
class TensorProductLayer(Layer):
    ...
    def __init__(self, prev_layer, n_out, params=None, bias=True, isTrainable = True):
        super().__init__(prev_layer)
        self._bias = bias
        n_in = self._input_shape[-1]

        if params is None:
            self.W = Weight((n_in, n_out), is_bias=False, train=isTrainable)
            if bias:
                self.b = Weight((n_out,), is_bias=True, mean=0.1, filler='constant', train=isTrainable)
        else:
            self.W = params[0]
            if bias:
                self.b = params[1]
    ...
    def set_output(self):
        print('shape shape ###############################3')
        print(self._prev_layer._output_shape)
