...
class Solver(object):
    ...
    def train_loss(self):
        if self._train_loss is None:
            ...
            self._train_loss = theano.function(
                [self.net.x, self.net.y], [self.net.loss, self.net.softloss.sum_exp_x, self.net.gru_s.output, self.net.res7.output, self.net.res8.output, self.net.res9.output, self.net.res10.output, self.net.conv11.output], updates=self.updates, profile=cfg.PROFILE)
    ...
    def train(self, train_queue, val_queue=None):
        ...
        """
        if cfg.INIT_R2N2:
            self.net.load_r2n2(cfg.half_init)
        """
        ...
        for train_ind in range(start_iter, cfg.TRAIN.NUM_ITERATION + 1):
            ...
            train_timer.tic()
            loss, exp_out, gru_s, res7, res8, res9, res10, conv11 = self.train_loss(batch_img, batch_voxel)
            train_timer.toc()
            
            print('exp out min %f max %f' %(np.amin(exp_out), np.amax(exp_out)) )
            print('param count %s' % (self.net.params_lst,))
            #print('gru_s min  %f max %f, res7 min %f max %f, res8 min %f max %f' %(np.amin(gru_s), np.amax(gru_s), np.amin(res7), np.amax(res7),  np.amin(res8), np.amax(res8)))
            #print('res9 min  %f max %f, res10 min %f max %f, conv11 min %f max %f' %(np.amin(res9), np.amax(res9), np.amin(res10), np.amax(res10),  np.amin(conv11), np.amax(conv11)))
            ...
            if train_ind % cfg.TRAIN.SAVE_FREQ == 0:
